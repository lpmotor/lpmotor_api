...
Запросы к API выполняются в формате:
...
Все запросы к API (кроме регистрации) требуют указания ключа, который идентифицирует пользователя (user_key).
Ключ можно получить при [регистрации пользователя](#user_reg).
...
400 | неверный запрос (ошибка в переданных данных) | проверьте формат и набор входящих параметров
403 | доступ запрещен | проверьте правильность переданного ключа. Также, возможно, передано неверное сочетание user_key и id объекта
404 | метод API не найден | проверьте правильность написания метода API и его версии
406 | сработали limit-ы на количество запросов | нужно повторить запрос через некоторое время
500 | ошибка на сервере | напишите нам, с указанием ошибки
...
Этот идентификатор выдается каждому пользователю при регистрации.
...
* __обязательно__
    * _partner_key_ - (hash) ключ партнера, 32 символа (генерируется в LPmotor)
    * _email_ - (string) почтовый ящик, который указывает ваш пользователь при регистрации

-

##### Ответ:

* _status_ - (string) статус выполнения операции
* _user_key_ - (hash) ключ, идентицифирующий пользователя в системе
* _adm_id_ - (int) идентификатор сервера, к которому нужно делать запросы, относящиеся к конкретному пользователю
...
Каждый элемент массива __list__ имеет формат:

    {
        ...
        "status": "1" //id статуса сайта
    }

-

##### <a name="site_status"></a>Статус сайта может иметь значения:
